...
const parts = input.trim().split("\n\n")[1].trim().split("\n").map(item => {
    const part = item.replace(/\{|\}/g, "").split(",");
    return part.reduce((result, item) => {
...
const checkRules = (workflows, part, key = "in") => {
    const rules = workflows[key];
    for({ condition, route } of rules) {
        let statement
        if(condition === undefined)
            statement = true;
        else {
            const partValue = Number(part[condition.part]);
            statement = eval(`${partValue}${condition.operator}${condition.value}`);
        }
        if(statement) {
            if(route === "A")
                return part;
            ...
            return checkRules(workflows, part, route);
        }
    }
...
let result = 0;
parts.forEach(part => {
    if(checkRules(workflows, part))
        result+= Object.values(part).reduce((result, item) => result+item, 0)
});
