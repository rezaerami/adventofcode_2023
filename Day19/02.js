...
const workflows = input.trim().split("\n\n")[0].trim().split("\n").reduce((result, item) => {
    const [key, rules] = item.split("{");
    result[key] = rules.replace("}", "").split(",").reduce((result, ruleItem) => {
        const [condition, route] = ruleItem.split(":");
        if(route) {
            const [part, operator, value] = condition.match(/([a-zA-Z]+)|([\W_])|(\d+)/g);
            result.push({condition: {part, operator, value: Number(value)}, route});
        }
        else
            result.push({condition: undefined, route: condition});
        return result
    }, [])
    return result
}, {});

const getDistinct = (workflows) => {
    const start = { route: 'in', x1: 1, x2: 4000, m1: 1, m2: 4000, a1: 1, a2: 4000, s1: 1, s2: 4000 };
    let result = 0;
    ...
    while(queue.length) {
        ...
        if (item.route === 'R') continue;
        if (item.route === 'A') {
            result += (1 + item.x2 - item.x1) * (1 + item.m2 - item.m1) * (1 + item.a2 - item.a1) * (1 + item.s2 - item.s1)
            continue
        }

        const workflow = workflows[item.route];
        for(const rule of workflow) {
            if(!rule.condition){
                queue.push({ ...item, route: rule.route });
                continue;
            }
            const {part, operator, value} = rule.condition;
            const next = { ...item, route: rule.route };

            if (operator === '<') {
                next[`${part}2`] = Math.min(next[`${part}2`], value - 1);
                queue.push(next);
                item[`${part}1`] = Math.max(item[`${part}1`], next[`${part}2`] + 1);
                continue;
            }

            next[`${part}1`] = Math.max(next[`${part}1`], value + 1);
            queue.push(next);
            item[`${part}2`] = Math.min(item[`${part}2`], next[`${part}1`] - 1);
        }
    }
...
console.log(getDistinct(workflows))