...
// Function to calculate the sum based on numeric digits in each string
const calculateSum = (result, item) => {
    // Extract numeric digits from the string using regex
    const match = item.match(/(\d)/gm);

    // If there are numeric digits, add the first and last digit to the result
    if (match) {
        const firstDigit = match[0];
        const lastDigit = match[match.length - 1];
        return result + parseInt(`${firstDigit}${lastDigit}`);
    }

    // If no numeric digits found, return the current result
    return result;
};
