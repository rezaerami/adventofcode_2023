...
const createGraph = (edges) => {
    ...
    const disconnect = (vertex, neighbor) => {
        if (graph.has(vertex)) {
            graph.get(vertex).delete(neighbor);
        }

        if (graph.has(neighbor)) {
            graph.get(neighbor).delete(vertex);
        }
    };
...
const generateFlourish = graph => {
    ...
    nodes.unshift(["name"]);
    fs.writeFileSync("./links.csv", links.join("\n"))
...
const { graph, disconnect } = createGraph(parsedInput);

generateFlourish(graph); // if you changed you input then generate the flourish input and visualize it on https://app.flourish.studio
// based on flourish visualization I found these three connections dqf->cbx, sds->hbr, xft->pzv
disconnect("dqf", "cbx");
disconnect("sds", "hbr");
disconnect("xft", "pzv");

// then we just count connections of dqf representing one side of graph vs cbx in other side of the graph
console.log(countConnections(graph, "dqf")*countConnections(graph, "cbx"));