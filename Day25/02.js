...
const randomGroup = () => Math.round(Math.random()) + 1;

const getDominantItem = (arr) => { // find most repeated item in an array
    const countMap = {};
...
const arrayEqual = (arr1, arr2) => { // compare to arrays
...
const createGraph = (edges) => {
    ...
    const initializeGroups = () => { // initializes a random group between 1 and 2
        graph.forEach((_, key) => {
            groups.set(key, randomGroup());
        })
    };

    const adjustGroup = (edges, vertex) => { // moves every node closer to its friends, so if a node is in group 1 but most of its friends are in 2, moves it in 2
        const currentGroup = groups.get(vertex)
        const newGroup = getDominantItem([...edges].map(item => groups.get(item)));
        if (currentGroup !== newGroup)
            groups.set(vertex, newGroup);
    }

    const isBridge = (edges) => { // checks if a node has a connection out of its group
        const neighborGroups = new Set([...edges].map(item => groups.get(item)));
        return neighborGroups.size > 1
    }

    const getGroupsCount = (groups) => [
        [...groups.values()].filter(item => item === 1).length,
        [...groups.values()].filter(item => item === 2).length,
    ];

    const getBridgeCount = () => {
        const result = [];
        graph.forEach((item, index) => {
            if (isBridge(item))
                result.push(index);
        })
        return result.length/2 // we have to divide it by 2 because it will give us bidirectional result which means two ends of a connection, while for two ends we should get 1
    };

    edges.forEach((edge) => {
        const [vertex, ...neighbors] = edge;
    ...
    let groupCount = getGroupsCount(groups);
    while (getBridgeCount() !== MAX_WIRES) { // checks if there are more than 3 bridges
        graph.forEach(adjustGroup);

        const newGroupCount = getGroupsCount(groups);
        if (arrayEqual(newGroupCount, groupCount) || newGroupCount.includes(0)) { // if randomization was in a way that it can't get three nodes out of it, it re randomizes everything and adjusts the group
            initializeGroups();
            continue;
        }
...
console.log(groupsCount[0] * groupsCount[1]);