...
input.split("\n").forEach(item => {
    if (item) {
        const [gameInfo, game] = item.split(/:\s/);
        const gameId = parseInt(gameInfo.match(/Game (\d+)/)[1]);
        const sets = game.split("; ");

        const info = { green: 0, red: 0, blue: 0 };
        ...
        for (const set of sets) {
            ...
            for (const cube of cubes) {
                ...
                if (info[color] < parseInt(count)) {
                    info[color] = parseInt(count);
                }
            }
        }

        const result = Object.values(info).reduce((count, total) => total * count, 1);
        games.push(result);
    }
});

const possibleGamesSum = games.reduce((sum, game) => sum + game, 0);
