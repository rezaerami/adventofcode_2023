...
rows.forEach((row, line) => {
    const gears = row.matchAll(/\*/gm);

    for (let gear of gears) {
        const index = gear.index;

        const start = index - 1 > 0 ? index - 1 : 0;
        const end = index + 1 < row.length ? index + 1 : row.length;
        const boundaries = [start, index, end];

        const checkTouch = (line) => {
            const result = [];

            if (line) {
                const digits = line.matchAll(/\d+/gm);

                for (let digit of digits) {
                    let isTouching = false;

                    for (let i = 0; i < digit[0].length; i++) {
                        if (boundaries.includes(digit.index + i)) {
                            isTouching = true;
                            break;
                        }
                    }
        ...
        const numbers = [...checkTouch(rows[line - 1]), ...checkTouch(rows[line]), ...checkTouch(rows[line + 1])];

        if (numbers.length > 1) {
            const result = numbers.reduce((mul, item) => mul * item, 1);
