const input = `
77.FFFF|-77FFL-F-7--F7.|-7.F|7FFF|-FL7-777F.L-FL7.L7--7.|.7-.J-7-J.F-L77F-7-L7F-F.F7-FL-|--7--JF|7.-7FFL7F-F7-F.FF|-F7LF-FLJFLF-J.F7F--F77..
...
const canConnect = (pipe, nextPipe) => {
    const pipeCell = map[pipe.y][pipe.x];
    const nextPipeCell = map[nextPipe.y][nextPipe.x];

    const pipePosition = getPosition(nextPipe, pipe);
    const nextPipePosition = getPosition(pipe, nextPipe);

    return connections[pipeCell]?.includes(nextPipePosition) && connections[nextPipeCell].includes(pipePosition)
}

const getPosition = (pipe, nextPipe) => {
    if (pipe.x === nextPipe.x && pipe.y < nextPipe.y) {
        return 'south';
    } else if (pipe.x === nextPipe.x && pipe.y > nextPipe.y) {
        return 'north';
    } else if (pipe.y === nextPipe.y && pipe.x < nextPipe.x) {
        return 'east';
    } else if (pipe.y === nextPipe.y && pipe.x > nextPipe.x) {
        return 'west';
    } else {
        return 'positions are not directly connected';
    }
}

const getNeighbors = (pipe, exclude) => {
    const { x, y } = pipe;
    const neighbors = [];

    const isExcluded = (item) => {
        return exclude && exclude.some(ex => ex.x === item.x && ex.y === item.y);
    };

    if (y > 0 && !isExcluded({ x, y: y - 1 })) {
        neighbors.push({ x, y: y - 1 });
    }
    if (y < map.length - 1 && !isExcluded({ x, y: y + 1 })) {
        neighbors.push({ x, y: y + 1 });
    }
    if (x > 0 && !isExcluded({ x: x - 1, y })) {
        neighbors.push({ x: x - 1, y });
    }
    if (x < map[y].length - 1 && !isExcluded({ x: x + 1, y })) {
        neighbors.push({ x: x + 1, y });
    }

    return neighbors;
}
const getDiagonalNeighbors = (pipe, exclude) => {
    const { x, y } = pipe;
    const neighbors = [];
    ...
    if (y > 0 && x > 0 && !isExcluded({ x: x - 1, y: y - 1 })) {
        neighbors.push({ x: x - 1, y: y - 1 });
    }
    if (y > 0 && x < map[y].length - 1 && !isExcluded({ x: x + 1, y: y - 1 })) {
        neighbors.push({ x: x + 1, y: y - 1 });
    }
    if (y < map.length - 1 && x > 0 && !isExcluded({ x: x - 1, y: y + 1 })) {
        neighbors.push({ x: x - 1, y: y + 1 });
    }
    if (y < map.length - 1 && x < map[y].length - 1 && !isExcluded({ x: x + 1, y: y + 1 })) {
        neighbors.push({ x: x + 1, y: y + 1 });
    }
...
const getPath = (start) => {
    ...
    let shouldGo = true;
    while (shouldGo) {
        const neighbors = getNeighbors(next, [prev]);
        for (let neighbor of neighbors) {
            if (canConnect(next, neighbor)) {
                prev = next;
                next = neighbor;
                path.push(next);
                break
...
const cellsWithinPolygon = (grid, polygon, newValue) => {
...
const start = getStartingPosition();
const path = getPath(start);

cellsWithinPolygon(result, path, "I");
path.forEach(item => result[item.y][item.x] = " ")


const visual = result.map(line => line.join("")).join("\n");
console.log(visual.match(/I/gm).length)